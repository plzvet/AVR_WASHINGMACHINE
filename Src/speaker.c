...
int Laundry_Bell_Tune[]={SO_01 ,MI_01 , MI_01 ,FA_01 ,RE_01 ,RE_01, DO_01, RE_01 ,MI_01, FA_01, SO_01, SO_01, SO_01,
SO_01, MI_01 ,MI_01, MI_01, FA_01, RE_01 ,RE_01 ,DO_01, MI_01, SO_01, SO_01,  MI_01, MI_01, MI_01,'\0'};


const int Laundry_Bell_Beats[]={BEAT_1_4,BEAT_1_4,BEAT_1_2,BEAT_1_4,BEAT_1_4,BEAT_1_2,BEAT_1_4,BEAT_1_4,BEAT_1_4,BEAT_1_4,BEAT_1_4,BEAT_1_4,BEAT_1_2,
...
void Laundry(int repeat)
{
	int i, note, beat;

	for(i = 0; i < repeat; i++)
	{
		for(int j = 0; Laundry_Bell_Tune[j] != 0; j++)  // 끝까지 음계가 있으면 반복
		{
			note = Laundry_Bell_Tune[j];      // 현재 음계
			beat = Laundry_Bell_Beats[j]; // 현재 박자

			if (note != 0)  // 0이 아닌 음이 있을 때만
			{
				OCR1A = note;         // PWM 주파수 설정 (음계에 따라)
				delay_ms(beat);      // 박자에 맞춰 대기
				OCR1A = 0;            // 소리 끄기
				delay_ms(10);        // 짧은 정지 후 다음 음으로
			}
		}
	}
}
